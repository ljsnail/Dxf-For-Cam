 ...
 void GeomForCut::CreatCutGuideLINE(GeomCloseHEAD*Phead)//输入每一个封闭环，然后对其进行切割 引刀线的添加。
{
	...
	newNode->m_GeomStandData.m_typegeomele = 6;//切割引刀线的标准
	//重点！！！
	//从此封闭环的头结点不再是第一个封闭环图元，而是切割引刀线
	//第二个才是原来共有的图元。
	Phead->FirstGeomele = newNode;//切割引刀线为首图元
	newNode->nextGeomeleNode = Fnode;//指向原来的首图元
	newNode->prevGeomeleNode = NULL;//自然它就成了第一个了
	Fnode->prevGeomeleNode = newNode;//原来前面指向的是NULL
	//如此封闭环就加上了切割引刀线
}
